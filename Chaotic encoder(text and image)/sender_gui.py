...
class SenderGUI:
    ...
    def _build_ui(self):
        ...
        ttk.Label(img_tab, text="Image (grayscale recommended):").pack(anchor='w')
    ...
    def simulate_image_and_preview(self):
        ...
        sys, enc = self._build_system_encoder()
        img = enc.load_image_grayscale(path)
        enc.system.simulate(600.0, 0.002)
        encrypted_bits, ks, order_list, shape, t, traj = enc.encode_image(img, t_span=600.0, dt=0.002)
        dec_img = enc.decode_image(encrypted_bits, ks, order_list, shape)
        self.last_image = {'original': img, 'encrypted_bits': encrypted_bits, 'keystream': ks, 'order': order_list, 'shape': shape, 't': t, 'traj': traj}
        show_image(self.root, img, title="Original Image (grayscale)")
        show_image(self.root, dec_img, title="Decrypted/Recovered (roundtrip)")
    ...
    def send_image(self):
        ...
        try:
            ...
            enc.system.simulate(600.0, 0.002)
            encrypted_bits, ks, order_list, shape, t, traj = enc.encode_image(img, t_span=600.0, dt=0.002)
            pkt = {'type':'image', 'encrypted_bits': encrypted_bits, 'keystream_sample': ks[:160], 'order': order_list, 'shape': shape, 'system': self.system_var.get(), 'init': (float(self.ic_x.get()), float(self.ic_y.get()), float(self.ic_z.get()))}
            data = pickle.dumps(pkt)
            self.client_socket.sendall(len(data).to_bytes(4,'big')); self.client_socket.sendall(data)
            self.last_image = {'original': img, 'encrypted_bits': encrypted_bits, 'keystream': ks, 'order': order_list, 'shape': shape, 't': t, 'traj': traj}
            self.log(f"Sent encrypted image (shape={shape}, bits={len(encrypted_bits)}).")
        except Exception as e:
