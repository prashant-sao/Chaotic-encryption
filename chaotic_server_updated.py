...
def main():
    ...
    selector.geometry("420x260")

    # --- MODIFICATION START ---
    # 1. Configure the window's grid to expand and center the frame
    selector.columnconfigure(0, weight=1)
    selector.rowconfigure(0, weight=1)
    # --------------------------

    frm = ttk.Frame(selector, padding=18)
    frm.grid(row=0, column=0, sticky=(tk.N, tk.S, tk.E, tk.W))

    # --- MODIFICATION START ---
    # 2. Configure the frame's grid to center the buttons/labels inside it
